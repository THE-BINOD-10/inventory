...
angular.module('urbanApp', ['datatables'])
  .controller('POApprovalTable',['$scope', '$http', '$state', '$timeout', 'Session', 'DTOptionsBuilder', 'DTColumnBuilder', 'colFilters', 'Service', ServerSideProcessingCtrl]);
    ...
    function rowCallback(nRow, aData, iDisplayIndex, iDisplayIndexFull) {
        ...
        $('td', nRow).bind('click', function() {
            $scope.$apply(function() {
              var wh_data = {'name': aData['name'], 'purchase_type': 'PO'};
...
//                  });
  ...
  vm.delete_config = function() {
    vm.service.apiCall('delete_pr_config/', 'POST', {'data':JSON.stringify(vm.model_data), 'type': 'pr_save'}).then(function(data){
      if(data.message) {
        if(data.data == "Deleted Successfully") {
          vm.service.refresh(vm.dtInstance);
          vm.close();
  ...
  vm.delete_range_levels_data = function(index, range_data, level_dat) {
    if(level_dat.data_id) {
      vm.service.apiCall('delete_pr_config/', 'POST', {'data_id': level_dat.data_id, 'type': 'pr_save'}).then(function(data){
        if(data.message) {
          if(data.data == "Deleted Successfully") {
            vm.service.refresh(vm.dtInstance);
          } else {
