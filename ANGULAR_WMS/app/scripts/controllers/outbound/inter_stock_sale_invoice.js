...
angular.module('urbanApp', ['datatables'])
  .controller('StockTransferInterInvoiceCtrl',['$scope', '$http', '$state', '$compile', '$timeout', 'Session','DTOptionsBuilder', 'DTColumnBuilder', 'DTColumnDefBuilder', 'colFilters', 'Service', '$modal', ServerSideProcessingCtrl]);
...
function ServerSideProcessingCtrl($scope, $http, $state, $compile, $timeout, Session, DTOptionsBuilder, DTColumnBuilder, DTColumnDefBuilder, colFilters, Service, $modal) {
    ...
    vm.service.apiCall("stock_transfer_inter_invoice_data/").then(function(data) {
        if(data.message) {
          vm.filters = {'datatable': 'StockInterInvoice', 'search0':'', 'search1':'', 'search2': '', 'search3': ''}
