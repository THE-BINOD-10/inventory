...
angular.module('urbanApp', ['datatables'])
  .controller('MRPExceptionCtrl',['$scope', '$http', '$state', '$compile', 'Session', 'DTOptionsBuilder', 'DTColumnBuilder', 'colFilters', 'Service', ServerSideProcessingCtrl]);
...
function ServerSideProcessingCtrl($scope, $http, $state, $compile, Session, DTOptionsBuilder, DTColumnBuilder, colFilters, Service) {
  ...
  vm.reports = {}
  var send = {};
  var name = 'get_mrp_exception_report';
  vm.service.apiCall("get_report_data/", "GET", {report_name: name}).then(function(data) {
  	if(data.message) {
        ...
        vm.service.get_report_dt(vm.empty_data, vm.report_data).then(function(datam) {
          ...
          vm.report_data['excel_name'] = 'get_mrp_exception_report'
        })
  	  }
  	}
  });
