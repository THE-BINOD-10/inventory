...
angular.module('urbanApp', ['datatables'])
  .controller('PRReportPerCtrl',['$scope', '$http', '$state', '$compile', 'Session', 'DTOptionsBuilder', 'DTColumnBuilder', 'colFilters', 'Service', ServerSideProcessingCtrl]);
...
function ServerSideProcessingCtrl($scope, $http, $state, $compile, Session, DTOptionsBuilder, DTColumnBuilder, colFilters, Service) {
  ...
  vm.report_data = {};
  vm.reports = {}
  vm.title = "PR Performance Report";

  vm.row_call = function(aData) {
    console.log('PR Print Blocked');
  }
  vm.toggle_po = function() {
    ...
    var name ='';
      ...
      name = 'get_pr_performance_report_dat';
    }
    vm.service.apiCall("get_report_data/", "GET", {report_name: name}).then(function(data) {
    	...
    	$state.go('app.reports.PRReportPerformance');
  	})
