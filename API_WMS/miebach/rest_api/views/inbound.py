...
@csrf_exempt
def get_pr_converted_po(start_index, stop_index, temp_data, search_term, order_term, col_num, request, user, filters, special_key=''):
    # filtersMap = {'purchase_type': 'PR', 'pending_po__open_po': None,  'quantity__gt': 0}
    filtersMap = {'purchase_type': 'PO', 'pending_po__open_po': None, 'pending_po__pending_prs__requested_user':request.user.id} # 'pending_pr__wh_user':user #'final_status': 'cancelled' Ignoring  cancelled status till reports created.
    # if request.user.id != user.id:
    #     currentUserLevel = ''
    #     currentUserEmailId = request.user.email
    ...
    lis = ['pending_po__pending_prs__full_pr_number', 'pending_po_id', "pending_po__pending_prs__requested_user__username",  "pending_po__pending_prs__creation_date",
            'sku__user', 'sku__user', 'pending_po__po_number', 'pending_po__supplier__supplier_id', 'pending_po__supplier__name',
            'pending_po__sku_category',  'total_qty', 'total_amt', 'creation_date',
    ...
    values_list = ['pending_po__requested_user', 'pending_po__requested_user__first_name',
                    ...
                    'pending_po__pending_prs__full_pr_number', "pending_po__pending_prs__requested_user__first_name", 
                    "pending_po__pending_prs__requested_user__username", 
                    "pending_po__pending_prs__creation_date",
                    'pending_po__pending_prs__sub_pr_number', 'pending_po__pending_prs__id']
    results = PendingLineItems.objects.filter(**filtersMap). \
    ...
    # POtoPRsMap = {}
    # pr_id_map = {}
    # for eachPO, pr_number, sub_pr_number, pr_id in approvedPRQs:
    #     if sub_pr_number:
    #         POtoPRsMap.setdefault(eachPO, []).append(str(pr_number) + '/' + str(sub_pr_number))
    #         pr_id_map.setdefault(eachPO, []).append()
    #     else:
    ...
    for result in results[start_index: stop_index]:
        ...
        pr_id = result["pending_po__pending_prs__id"]
        if result["pending_po__pending_prs__sub_pr_number"]:
            approvedPRs = result["pending_po__pending_prs__full_pr_number"] + "/" + str(result["pending_po__pending_prs__sub_pr_number"])
        else:
            approvedPRs = result["pending_po__pending_prs__full_pr_number"]
        ...
        temp_data['aaData'].append(OrderedDict((
                                                ...
                                                ('Store', store),
                                                ('Department', department),
                                                ('PR Raise By', result['pending_po__pending_prs__requested_user__first_name']),
                                                ('PR Request User', result['pending_po__pending_prs__requested_user__username']),
                                                ('PR Created Date', datetime.datetime.strftime(result['pending_po__pending_prs__creation_date'], '%d-%m-%Y') if result['pending_po__pending_prs__creation_date'] else ""),
                                                ('PO Raise By', result['pending_po__requested_user__first_name']),
                                                ('PO Requested User', result['pending_po__requested_user__username']),
                                                ('PO Validation Status', result['pending_po__final_status'].title()),
                                                ('PO Pending Level', '%s Of %s' %(result['pending_po__pending_level'], lastLevel)),
                                                ('PO LevelToBeApproved', result['pending_po__pending_level']),
                                                ('PO To Be Approved By', validated_by),
                                                ('PO Last Updated By', last_updated_by),
                                                ('PO Last Updated At', last_updated_time),
                                                ('Remarks', last_updated_remarks),
                                                ('id', pr_id),
                                                ('DT_RowClass', 'results'))))
        count += 1
...
def get_approval_pending_enquiry_results(start_index, stop_index, temp_data, search_term, order_term, col_num, request, user, filters):
    po_itemVals = ['supplier__supplier_id', 'product_category', 'final_status', 'pending_level',
                'requested_user__first_name', 'wh_user__first_name', 'po_number', 'supplier__name',
                'creation_date', 'delivery_date', 'prefix', 'full_po_number']
    pr_itemVals = ['product_category', 'final_status', 'pending_level',
                'requested_user__first_name', 'wh_user__first_name', 'pr_number',
                'creation_date', 'delivery_date', 'prefix', 'full_pr_number']
    ...
    for enqObj in enqQs:
        ...
        if itemDetQs.exists():
            ...
            continue
        purchase_date = itemDets['creation_date'].strftime('%d-%m-%Y')
        purchase_delivery_date = itemDets['delivery_date'].strftime('%d-%m-%Y')
        dateInPO = str(purchase_date).split(' ')[0].replace('-', '')
        purchase_reference = itemDets[full_purchase_number] #'%s%s_%s' % (itemDets['prefix'], dateInPO, itemDets['po_number'])
        ...
        enquiryDict = OrderedDict((
                                ('id', enqObj.id),
                                ('Purchase Number', purchase_reference),
                                ('Product Category', itemDets['product_category']),
                                ('Supplier ID', itemDets.get('supplier__supplier_id', '')),
                                ('Supplier Name', itemDets.get('supplier__name', '')),
                                ('PO Created Date', purchase_date),
                                ('PO Delivery Date', purchase_delivery_date),
...
def get_pending_enquiry(request, user=''):
    ...
    if enqQs.exists():
        ...
        if pendingObjModel == 'pendingPO':
            model_name = PendingPO
            lineItemsStr = 'pending_polineItems'
            purchaseApprls = 'pending_poApprovals'
        elif pendingObjModel == 'pendingPR':
            model_name = PendingPR
            lineItemsStr = 'pending_prlineItems'
            purchaseApprls = 'pending_prApprovals'
        else:
            pass

        pendingObj = model_name.objects.get(id=pendingObjId)
        lineItemQs = getattr(pendingObj, lineItemsStr)
        total_data = []
        ser_data = []
        levelWiseRemarks = []
        pr_delivery_date = ''
        ...
        purchaseApprQs = getattr(pendingObj, purchaseApprls)
        allRemarks = purchaseApprQs.exclude(status__in=['', 'resubmitted']).values_list('level', 'validated_by', 'remarks')
        pendingLevelApprovers = list(purchaseApprQs.filter(status__in=['pending', '']).values_list('validated_by', flat=True))
        ...
        validated_users = list(purchaseApprQs.filter(status='approved').values_list('validated_by', flat=True).order_by('level'))
        ...
        purchase_data = {
                    ...
                    'purchase_created_date': pr_created_date, 'warehouse': pendingObj.wh_user.first_name,
                    'data': ser_data, 'levelWiseRemarks': levelWiseRemarks, 'is_approval': 1,
                    'validateFlag': validateFlag, 'validated_users': validated_users
                 }
        if pendingObjModel == 'pendingPO':
            purchase_data['supplier_id'] = pendingObj.supplier.supplier_id
            purchase_data['supplier_name'] = pendingObj.supplier.name
        ...
        pendingEnqData.update(purchase_data)
...
def get_receive_po_datatable_filters(user, filters, request):
    ...
    if filters.get('search_0', None):
        ...
        po_ids = PurchaseOrder.objects.filter(po_number__icontains=col_val,
                                              open_po__sku__user__in=user,
        ...
        stock_results = STPurchaseOrder.objects.exclude(po__open_po__isnull=True).exclude(
            ...
            filter(po__po_number__icontains=col_val,
                   open_st__sku__user__in=user,
        ...
        rw_results = RWPurchase.objects.exclude(purchase_order__open_po__isnull=True). \
            ...
            filter(purchase_order__po_number__icontains=col_val,
                   rwo__vendor__user__in=user,
...
def get_filtered_purchase_order_ids(request, user, search_term, filters, col_num, order_term):
    ...
    st_purchase_list = ['po__order_id', 'po__order_id', 'open_st__warehouse__username', 'po__order_id',
                        ...
                        'po__order_id', 'open_st__warehouse__username', 'po__order_id', 'po__order_id', 'po__order_id', 'po__po_number']
    ...
    rw_order_qty = rw_results_objs.values_list('purchase_order__order_id', 'purchase_order__prefix', 'purchase_order__po_number').distinct().annotate(total_order_qty=Sum('rwo__job_order__product_quantity'))
    rw_receive_qty = rw_results_objs.values_list('purchase_order__order_id', 'purchase_order__prefix', 'purchase_order__po_number').distinct().annotate(total_received_qty=Sum('purchase_order__received_quantity'))
...
@csrf_exempt
def get_confirmed_po_doa(start_index, stop_index, temp_data, search_term, order_term, col_num, request, user, filters):
    # lis = ['PO No', 'PO No', 'PO Reference', 'Customer Name', 'Order Date', 'Expected Date',
    #        'Total Qty', 'Receivable Qty', 'Received Qty',
    #        'Remarks', 'Supplier ID/Name', 'Order Type', 'Receive Status', 'SR Number', 'Requested User', 'Status' ]
    lis = ['requested_user_id', 'po_no', 'po_reference', 'customer_name', 'total_qty',
    ...
    for row in mapping_results[start_index: stop_index]:
        ...
        if(product_category):
            ...
            total_order_qty=result.get("total_order_qty",0)
            total_receivable_qty=result.get("total_receivable_qty",0)
            total_received_qty=result.get("total_received_qty",0)
            supplier_id_name=result.get("supplier_id_name","")
            expected_date=result.get("expected_date","")
            order_type=result.get("order_type","")

        temp_data['aaData'].append(OrderedDict((('po_no', result['po_number']), ('po_reference', ""),
                                                ('supllier_id_name', supplier_id_name),
                                                ('total_qty', total_order_qty),
                                                ('receivable_qty', total_receivable_qty),
...
@csrf_exempt
def verify_putaway_data(results):
    formation_results = []
    for result in results:
        if POLocation.objects.filter(purchase_order__po_number=result['po_number'], receipt_number=result['sellerposummary__receipt_number'], purchase_order__sellerposummary__grn_number= result['sellerposummary__grn_number'], status=1, quantity__gt=0).exists():
...
def get_order_data(start_index, stop_index, temp_data, search_term, order_term, col_num, request, user):
    ...
    for result in results[start_index:stop_index]:
        if po_dict.filter(order_id=result['order_id'], open_po__sku__user__in=user_ids, prefix=result['prefix'], po_number=result['po_number'], sellerposummary__grn_number=result['sellerposummary__grn_number']).exists():
            supplier = PurchaseOrder.objects.filter(order_id=result['order_id'], open_po__sku__user__in=user_ids, prefix=result['prefix'], po_number=result['po_number'], sellerposummary__grn_number=result['sellerposummary__grn_number'], sellerposummary__receipt_number=result['sellerposummary__receipt_number'])[0]
        ...
        temp_data['aaData'].append({'DT_RowId': supplier.order_id, 'Supplier ID': order_data['supplier_id'],
                                    ...
                                    'grn_number': result['sellerposummary__grn_number'],
                                    'receipt_number': result['sellerposummary__receipt_number'],
                                    'Order Date': get_local_date(request.user, grn_time_date),
...
def generated_pr_data(request, user=''):
    ...
    if not record[0].wh_user.userprofile.currency:
        ...
        warehouse_currency = record[0].wh_user.userprofile.currency.currency_code
        tax_display, msg, cu_code, currency_words = get_currency_tax_display(record[0].wh_user)
        if msg:
    ...
    for rec in lineItems:
        sku_desc_edited, service_stdate, service_edate, temp_price, temp_tax, temp_cess_tax = ['']*6
        sku_id, sku_code, sku_desc, qty, price, uom, apprId, cgst_tax, sgst_tax, igst_tax, cess_tax = rec
    ...
    return HttpResponse(json.dumps({'supplier_id': supplier_id, 'supplier_name': supplier_name, 'supplier_payment_desc': supplier_payment_desc,
                                    ...
                                    'warehouse_currency': warehouse_currency,
...
def print_pending_po_form(request, user=''):
    ...
    if pendingPurchaseQs.exists():
        ...
        wh_user =  pendingPurchaseObj.wh_user
        if is_actual_pr == 'true':
    ...
    tax_display, msg, cu_code, currency_words = get_currency_tax_display(wh_user)
    if msg:
        return HttpResponse(msg)
    ...
    for order in lineItems.values(*values_list):
        ...
        if tax_display:
            ...
            po_temp_data = [order['sku__sku_code'], order['sku__sku_desc'], supplier_code,
                        order['quantity'], order['measurement_unit'], current_price, amount, total_sku_amt]
        ...
        if convertPRtoPO and supplier:
            ...
            if skuTaxes:
                ...
                if taxes:
                    ...
                    value['cess_tax'] = taxes[0]['cess_tax']
                if skuTaxVal.get('sku_supplier_price', ''):
                    value['price'] = skuTaxVal.get('sku_supplier_price', '')
                else:
                    value['price'] = skuTaxVal['mrp']
        data_id = value['data_id']
        if data_id and PendingLineItems.objects.filter(id=data_id).exists():
            ...
            if value.has_key('temp_cess_tax'):
                misc_json['temp_cess_tax'] = value['temp_cess_tax']
            if misc_json:
    ...
    try:
        ...
        for supplier, all_skus in supplierSKUMapping.items():
            ...
            for sku_code in all_skus:
                ...
                tax, sgst_tax, cgst_tax, igst_tax, cess_tax, price, total = [0]*7
                sku_id = SKUMaster.objects.filter(sku_code=sku_code, user=pr_user.id)
                if not sku_id:
                    continue
                ...
                if pr_supplier_data:
                    ...
                    try:
                        ...
                        tax = 0
                    try:
                        cess_tax = float(json_data.get('cess_tax', 0))
                    except:
                        log.info("Convert PR to Failed due to cess tax: %s and LineItem:%s" %(json_data.get('cess_tax', ''), lineItemId))
                        cess_tax = 0
                    if tax_type == 'inter_state':
                        igst_tax = tax
                        cgst_tax = 0
                ...
                pendingLineItems['cess_tax'] = cess_tax
...
def get_pr_preview_data(request, user=''):
    ...
    skuSupplierMap = {}
    tax_display = True
    if lineItemsQs.exists():
        tax_display, msg, cu_code, currency_words = get_currency_tax_display(lineItemsQs[0].pending_pr.wh_user)
        if msg:
            return HttpResponse(msg)
        ...
        for each_pr in pr_ids:
            ...
            if not existingApprovedPR.exists():
                if poItems == prItems:
                    ...
                    # TODO..On Approved entry has to be created...dev is pending
                    # prApprovalsMap = {
                    #     'purchase_number': prObj.pr_number,
                    #     'pr_user': prObj.wh_user,
                    #     'level': 'level0',
...
def submit_pending_approval_enquiry(request, user=''):
    log.info("Enquiry Submission for pending purchase by user %s and request params are %s"
    ...
    try:
        if is_purchase_request == 'true':
            ...
            purchase_type = 'PO'
            lineItems_attr = 'pending_polineItems'
            purchase_number_attr = 'po_number'
        else:
            permission_name = 'pending pr'
            model_name = PendingPR
            master_type = 'pendingPR'
            purchase_type = 'PR'
        ...
        if lineItems.exists():
            ...
            <p>Pending Level : %s </p> \
            <p>Enquiry : %s </p> \
            <p>%s : %s</p>" %(purchase_type, purchase_type, po_reference, totalAmt, warehouseName, purchase_type,
        ...
        import traceback
        log.debug(traceback.format_exc())
        log.info("Enquiry Submission for pending Purchase " + str(request.POST.dict()) + \
...
def get_supplier_data(request, users=''):
        ...
        if po_quantity > 0:
            ...
            if temp_jsons.exists():
                for temp_json_obj in temp_jsons:
                    if(model_id):
                        temp_json = json.loads(temp_json_obj.json_data)
                    else:
    ...
    if purchase_orders:
        ...
        if temp_json.exists():
            if(model_id):
                temp_json = json.loads(temp_json[0].json_data)
            else:
    ...
    return HttpResponse(json.dumps({'data': orders, 'po_id': order_id, 'options': REJECT_REASONS, \
                                    ...
                                    'remainder_mail': remainder_mail, 'invoice_number': invoice_number, 'tax_display': tax_display,
    ...
    try:
        ...
        if asn_obj:
            ...
            data_dict = {'asn_number':asn_number, 'po_num': po_obj[0].po_number, 'supplier_name':supplier_obj.name, 
                        'supplier_id':supplier_id_name, 'asn_date': get_local_date(user, asn_obj.creation_date),
                        'qr_data':True}
...
def send_for_approval_confirm_grn(request, confirm_returns='', user=''):
    ...
    if not confirm_returns:
        ...
        data_dict = dict(request_data.iterlists())
    else:
        data_dict = confirm_returns
    log.info('Request params for ' + user.username + ' is ' + str(data_dict))
    try:
        wms_validation_status = validate_grn_wms(user, data_dict)
        if wms_validation_status:
            return HttpResponse(wms_validation_status)
        po = PurchaseOrder.objects.filter(id=data_dict['id'][0])
        prQs = PendingPO.objects.filter(wh_user=user.id, full_po_number=po[0].po_number)
        if(prQs[0].pending_prs.all()):
            ...
            if file_obj:
                ...
                # else:
                #     doa_obj = doaQs[0]
                #     doa_obj.json_data = json.dumps(po_data)
                #     doa_obj.save()
    except Exception as e:
        import traceback
        log.debug(traceback.format_exc())
        log.info("Check Generating GRN failed for params " + str(data_dict) + " and error statement is " + str(e))
...
def get_received_orders(request, user=''):
    ...
    full_po_number = request.GET['po_number']
    full_grn_number = request.GET['grn_number']
    grn_receipt_no = request.GET['receipt_number']
    purchase_orders = PurchaseOrder.objects.filter(order_id=supplier_id, open_po__sku__user=user.id, prefix=prefix, sellerposummary__receipt_number=grn_receipt_no,
                                                   open_po__sku_id__in=sku_master_ids, po_number=full_po_number, sellerposummary__grn_number=full_grn_number). \
    ...
    for order in purchase_orders:
        order_id = order.id
        order_data = get_purchase_order_data(order)
        po_location = POLocation.objects.filter(purchase_order_id=order_id, status=1, location__zone__user=user.id, receipt_number=grn_receipt_no)
    ...
    try:
        if is_purchase_request == 'true':
            ...
            if prQs:
                ...
                product_category = prObj.product_category
                user = prObj.wh_user
                if prObj.pending_polineItems.filter().exists():
                    pending_po_line_entries=prObj.pending_polineItems.filter()
        if not sales_data:
            myDict = dict(request.POST.iterlists())
        else:
            myDict = sales_data
        ...
        if tax_display:
            ...
            table_headers = ['SKU Code', 'HSN Code', 'Supplier Code', 'Desc', 'Delivery Schedule', 'Qty', 'UOM', 'Unit Price', 'MRP', 'Amt', 'Total']
        # if ean_flag:
        #     table_headers.insert(1, 'EAN')
        if show_cess_tax and tax_display:
            table_headers.insert(table_headers.index('IGST (%)'), 'CESS (%)')
        if show_apmc_tax and tax_display:
            table_headers.insert(table_headers.index('IGST (%)'), 'APMC (%)')
        if display_remarks == 'true':
            table_headers.append('Remarks')
        order_id = None
        for key, value in all_data.iteritems():
            ...
            if industry_type == 'FMCG':
                ...
                if pending_po_line_entries:
                    sku_pending_line = pending_po_line_entries.filter(sku__sku_code=wms_code)
                ...
                if not po_temp_data:
                    ...
                    if tax_display:
                        ...
                        po_temp_data = [wms_code, purchase_order.sku.hsn_code, supplier_code, purchase_order.sku.sku_desc, '', purchase_order.order_quantity,
                                po_suggestions['measurement_unit'],
                                current_price, round(purchase_order.mrp/ order.currency_rate, 2), amount,
                                total_sku_amt
                                ]
                    if show_cess_tax and tax_display:
                        po_temp_data.insert(table_headers.index('CESS (%)'), purchase_order.cess_tax)
