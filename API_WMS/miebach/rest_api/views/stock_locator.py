...
def get_stock_results(start_index, stop_index, temp_data, search_term, order_term, col_num, request, user, filters):
    ...
    for ind, data in enumerate(master_data[start_index:stop_index]):
        ...
        if quantity:
            ...
            total_wms_qty_sku_unit_price = sum(
                wms_code_obj_sku_unit_price.annotate(stock_value=Sum(F('quantity') * F('sku__cost_price'))).values_list(
                    'stock_value', flat=True))
...
def ba_to_sa_calculate_now(request, user=''):
    ...
    try:
        ...
        for data in master_data:
            ...
            if sku_avail_qty < 1.5 * peak:
                remarks = 'Available Quantity is more than Min Stock'
                replenishment_qty = 0
                ba_stock_objs = []
                needed_qty = 0
            else:
                replenishment_qty = required_inventory - sku_avail_qty
            ...
            if ba_stock_dict:
                total_ba_stock = ba_stock_dict[
                    'total_quantity']  # ba_stock_objs.aggregate(Sum('sellerstock__quantity'))[
                # 'sellerstock__quantity__sum']
