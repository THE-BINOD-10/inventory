...
def book_trial(request, user=''):
    ...
    if not book_instance:
        ...
        book_trial = BookTrial.objects.create(full_name=full_name, email=email, contact=contact, company=company,
                                              hash_code=hash_code,
                                              status=1)
        html = '<p> Hi %s, </p> <p> Thanks for using our demo version . This is valid for 30 Days. </p> To signup please click given link </p><p>link: https://%s/#/signup?hashcode=%s </p>' % (
        full_name, "go.stockone.in", hash_code)
        send_mail([email], 'StockOne 30 day trial', html)
        status = "Added Successfully"

        subject = "NEW FREE TRIAL SIGNUP"
        body = "<p> Following user requested for FREE TRIAL</p>  <p> NAME : %s</p> <p> Email : %s</p> <p> CONTACT: %s</p> <p> COMPANY: %s</p>" % (
        full_name, email, contact, company)
...
def inform_mail(subject, body):
    ...
    recipient = ['sreekanth@mieone.com', 'vimal.nair@miebach.com', 'karthik@headrun.com', 'roopal@mieone.com',
                 'alkesh.karamkar@miebach.com',
                 'malini@mieone.com', 'sameena@mieone.com']
    try:
...
def contact_us(request):
    ...
    html = "<p>Hi %s </p> <p> Thanks for contacting us. </p> <p> Our team will get back to you shortly. </p> <p> Thanks, </p> <p> Mieone Team </p>" % (
    full_name)
    send_mail([email], 'StockOne Query Auto Response', html)
    status = "Success"
    subject = "NEW QUERY GENERATED"
    body = "<p> Following user created a query</p>  <p> NAME : %s</p> <p> Email : %s</p> <p> CONTACT: %s</p> <p> COMPANY: %s</p> <p> QUERY: %s</p>" % (
    full_name, email, contact, company, query)
    inform_mail(subject, body)
