...
class TallyAPI:
	...
	tally_config = TallyConfiguration.objects.filter(user=self.user_id).values(\
                        "company_name", "stock_group", "stock_category", "maintain_bill",\
	...
	tally_config = self.tally_configuration()
        ...
        for obj in seller_summary:
            ...
            if obj["order__original_order_id"]: s_obj["orders"] = [obj["order__original_order_id"]]
            else: s_obj["orders"] = ["".join([obj["order__order_code"], str(obj["order__order_id"])])]
            ...
            COD = CustomerOrderSummary.objects.filter(order=obj["order__order_id"],\
                                                      order__user = self.user_id).values(\
	    ...
	    s_obj.setdefault("party_ledger", {})
            ...
            vat_ledger = VatLedgerMapping.objects.filter(tax_percentage = party_ledger_total_tax,\
                                            tax_type = "sales", user = self.user_id)
