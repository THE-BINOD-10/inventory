...
def sku_excel_upload(request, reader, user, no_of_rows, no_of_cols, fname, file_type='xls', attributes={},
    ...
    for row_idx in range(1, no_of_rows):
        ...
        temp_dict += ['size_type', 'hot_release', 'base_uom', 'uom_type', 'uom', 'conversion']
        ...
        if instanceName.__name__ in ['AssetMaster', 'ServiceMaster', 'OtherItemsMaster', 'TestMaster'] and not sku_data:
            ...
            sku_data.save()
        if sku_data:
            uom_dict['sku_code'] = sku_data.sku_code
            sku_data.save()
        ...
        if not sku_data:
            uom_dict['sku_code'] = data_dict['wms_code']
            data_dict['sku_code'] = data_dict['wms_code']
            ...
            #sku_data = sku_master
        name = '%s-%s' % (uom_dict['uom'], str(int(uom_dict['conversion'])))
        uom_dict['name'] = name
        uom_dict['company_id'] = company_id
        uom_obj = UOMMaster.objects.filter(**uom_dict)
        if not uom_obj:
            ...
            for sku_group_key, sku_valid_data in all_data.items():
                ...
                for group_key, valid_data in sku_valid_data['data'].items():
                    ...
                    if valid_data['base_uom_qty'] < putaway_pending_qty:
                        for row_idx in valid_data.get('indexes', []):
...
@login_required
@get_admin_user
def closing_stock_in_bulk_form(request, user=''):
    excel_file = request.GET['download-closing-stock-file']
    if excel_file:
        return error_file_download(excel_file)
    excel_mapping = copy.deepcopy(CLOSING_STOCK_FILE_MAPPING)
    excel_headers = excel_mapping.keys()
    wb, ws = get_work_sheet('Closing Stock', excel_headers)
    return xls_to_response(wb, '%s.closing_stock_form.xls' % str(user.username))


@csrf_exempt
def validate_closing_stock_in_bulk_form(request, reader, user, no_of_rows, no_of_cols, fname, file_type):
    index_status = {}
    data_list = []
    inv_mapping = copy.deepcopy(CLOSING_STOCK_FILE_MAPPING)
    inv_res = dict(zip(inv_mapping.values(), inv_mapping.keys()))
    excel_mapping = get_excel_upload_mapping(reader, user, no_of_rows, no_of_cols, fname, file_type,
                                                 inv_mapping)
    if user.userprofile.warehouse_type == 'ADMIN' and request.user.is_staff:
        ...
        users = [user.id]
        all_users = check_and_get_plants_depts(request, users)
    all_data = OrderedDict()
    user_skus = {}
    dept_mapping = copy.deepcopy(DEPARTMENT_TYPES_MAPPING)
    ...
    for row_idx in range(1, no_of_rows):
            ...
            if key == 'closing_date':
                ...
                if cell_data:
                    if cell_data not in dept_mapping_res.keys():
                        ...
                        department = dept_users.filter(userprofile__stockone_code=dept)
                        if department:
        ...
        if not index_status:
            ...
            user_skus.setdefault(data_dict['user'].id, [])
            user_skus[data_dict['user'].id].append(data_dict['sku'].sku_code)
            all_data.setdefault(sku_cond, {'stocks': StockDetail.objects.none(), 'sku': data_dict['sku'],
                                        'closing_date': data_dict['closing_date'],
    ...
    if not index_status:
        ...
        for user_id, skus in user_skus.items():
            ...
            for remaining_sku in remaining_sku_list:
                ...
                # all_data.setdefault(sku_cond, {'stocks': sku_remaining_objs,
                #                                'data': {}})
                all_data[sku_cond] = {'stocks': StockDetail.objects.none(), 'sku': sku_remaining_objs[0].sku,
                                    'closing_date': data_dict['closing_date'],
                                    'user': data_dict['user'], 'base_uom_qty': 0,
                                    'indexes': []}
    ...
    if index_status and file_type == 'csv':
        ...
        if file_path:
            f_name = file_path
        return f_name, all_data, all_stocks
...
@login_required
@get_admin_user
def closing_stock_in_bulk_upload(request, user=''):
    from dateutil.relativedelta import relativedelta
    fname = request.FILES['files']
    try:
        fname = request.FILES['files']
        ...
        return HttpResponse('Invalid File')
    status, data_list, all_stocks = validate_closing_stock_in_bulk_form(request, reader, user, no_of_rows,
                                                     no_of_cols, fname, file_type)
    if status != 'Success':
        return HttpResponse(status)
    try:
        with transaction.atomic('default'):
            ...
            for group_key, final_data in data_list.items():
                ...
                if not sku_stocks:
                    if sku.zone:
                        location = sku.zone.locationmaster_set.filter()[0]
                    else:
                        ...
                        if not put_zone:
                            ...
                            put_zone = put_zone[0]
                        location = put_zone.locationmaster_set.filter()[0]
...
@login_required
@get_admin_user
def closing_stock_form(request, user=''):
    excel_file = request.GET['download-closing-stock-file']
    if excel_file:
        return error_file_download(excel_file)
    excel_mapping = copy.deepcopy(CLOSING_STOCK_FEATURE_FILE_MAPPING)
    excel_headers = excel_mapping.keys()
    wb, ws = get_work_sheet('Closing Stock', excel_headers)
    return xls_to_response(wb, '%s.closing_stock_form.xls' % str(user.username))
...
def validate_closing_stock_form(request, reader, user, no_of_rows, no_of_cols, fname, file_type):
    ...
    for row_idx in range(1, no_of_rows):
        ...
        if data_dict.get('sku', ''):
            ...
            pcf = pcf if pcf else 1
            #stock_pquantity = stock_quantity/pcf
            if stock_quantity < data_dict['base_uom_quantity']:
                #data_dict['purchase_uom_quantity'] = round(stock_pquantity,4)
            ...
            for final_data in data_list:
                try:
                    ...
                    if replenushment_obj.exists():
                        ...
                        replenushment_obj.mrp_receiver = final_data.get('mrp_receiver', '').email_id if final_data.get('mrp_receiver', '') else ''
                        ...
                        replenushment['mrp_receiver'] = final_data.get('mrp_receiver', '').email_id if final_data.get('mrp_receiver', '') else ''
                        ReplenushmentMaster.objects.create(**replenushment)
                    staff_master = final_data.get('mrp_receiver', '')
                    if staff_master:
                        staff_master.mrp_user=True
                        staff_master.save()
                except:
                    pass
    except Exception as e:
